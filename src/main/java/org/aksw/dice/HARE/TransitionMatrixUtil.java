  ...
  public void getDimensionValues(Model data) {
    ...
    while (iter.hasNext()) {
      ...
      if (t.getObject() != null) {
        ...
        if (t.getObject().isResource()) this.entitySet.add(t.getObject().asResource());
        else this.entitySet.add(ResourceFactory.createResource(t.getObject().toString()));
      }
    }
        ...
        if ((!entityList.isEmpty()) && (!tripleList.isEmpty())) {
          for (Resource res : entityList) {
            ...
            for (Statement trip : tripleList) {
              ...
              } else if ((trip.getSubject().equals(res))
                  || (trip.getPredicate().equals(res))
