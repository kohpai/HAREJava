...
public class RDFReadWriteHandler {
  ...
  public void writeRDFResults(
    ...
    for (Statement triple : tripleList) {
      ...
      if (triple.getObject().isLiteral()) {
        ...
        String pseudoName =
            index < 0 ? name : name.substring(0, index); // in case of string literal without type
        String res = null;
        ...
        outputModel.addLiteral(
            triple.getObject().asResource(),
            hare,
      ...
      outputModel.addLiteral(
          triple.getPredicate().asResource(),
          hare,
  ...
  public void writePageRankResults(
      Matrix S_n,
      ArrayList<Statement> tripleList,
      ArrayList<Resource> entityList,
      String datasetname) {
  ...
  public Model readDataUsingThreads(final String filename) {
    ...
    while (iter.hasNext()) {
      ...
      if (next.getObject().isLiteral())
        model.add(
            new ResourceImpl(next.getSubject().toString()),
        ...
        model.add(
            new ResourceImpl(next.getSubject().toString()),
            new PropertyImpl(next.getPredicate().getURI()),
            new ResourceImpl(next.getObject().toString()));
    }
